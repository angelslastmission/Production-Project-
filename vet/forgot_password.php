...
function ensure_vet_password_resets_table(mysqli $conn): bool {
  $sql = "
    CREATE TABLE IF NOT EXISTS vet_password_resets (
...
function build_vet_reset_url(string $token, string $email): string {
  $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
  $host = $_SERVER['HTTP_HOST'] ?? 'localhost';
  $basePath = rtrim(str_replace('\\', '/', dirname(dirname($_SERVER['SCRIPT_NAME'] ?? '/vet/forgot_password.php'))), '/');
  if ($basePath === '.' || $basePath === '/') {
    $basePath = '';
  }

  return $scheme . '://' . $host . $basePath . '/vet/reset_password.php?email=' . urlencode($email) . '&token=' . urlencode($token);
}

function send_vet_reset_email(string $toEmail, string $toName, string $resetUrl, string $gmailUsername, string $gmailAppPassword): bool {
  ...
  $mail->addAddress($toEmail, $toName ?: 'Veterinarian');
  $mail->isHTML(true);
  $mail->Subject = 'PetCura - Reset your veterinarian password';
  $mail->Body = '<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1f2937;">
    <h2 style="margin-bottom:12px;">Reset your veterinarian password</h2>
    <p>We received a request to reset your PetCura veterinarian password.</p>
  ...
  $mail->AltBody = 'Reset your veterinarian password: ' . $resetUrl;

  return $mail->send();
}

if (!ensure_vet_password_resets_table($conn)) {
  $error = 'Unable to prepare password reset storage.';
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $rate_action = 'forgot_vet';
  $count_attempt = false;
  ...
  if ($error === '' && $email === '') {
    ...
    $stmt = mysqli_prepare($conn, "SELECT id, first_name, last_name, email FROM users WHERE email = ? AND role = 'vet' AND is_active = 1 LIMIT 1");

    if ($stmt) {
      mysqli_stmt_bind_param($stmt, 's', $email);
      mysqli_stmt_execute($stmt);
      $result = mysqli_stmt_get_result($stmt);
      $vet = $result ? mysqli_fetch_assoc($result) : null;
      ...
      if ($vet) {
        ...
        $insertStmt = mysqli_prepare($conn, "INSERT INTO vet_password_resets (vet_id, email, token_hash, expires_at) VALUES (?, ?, ?, ?)");
        if ($insertStmt) {
          mysqli_stmt_bind_param($insertStmt, 'isss', $vet['id'], $vet['email'], $tokenHash, $expiresAt);
          if (mysqli_stmt_execute($insertStmt)) {
            $resetUrl = build_vet_reset_url($token, (string)$vet['email']);

            try {
              send_vet_reset_email((string)$vet['email'], trim((string)$vet['first_name'] . ' ' . (string)$vet['last_name']), $resetUrl, $gmailUsername, $gmailAppPassword);
