...
if ($error === '' && $token !== '' && $email !== '') {
    $stmt = mysqli_prepare(
        $conn,
        "SELECT id, vet_id, email, token_hash, expires_at
      FROM vet_password_resets
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $showForm && $resetRow && $rate_error === '' && $csrf_valid) {
    ...
    if ($new_password === '' || $confirm_password === '') {
        ...
        $userStmt = mysqli_prepare($conn, "SELECT password FROM users WHERE id = ? AND role = 'vet' LIMIT 1");
        if ($userStmt) {
            mysqli_stmt_bind_param($userStmt, 'i', $resetRow['vet_id']);
            ...
            if (!$userRow) {
                $error = 'Vet account not found.';
            } elseif (password_verify($new_password, (string)$userRow['password'])) {
                $error = 'New password must be different from your current password.';
            } else {
                $newHash = password_hash($new_password, PASSWORD_DEFAULT);
                $updateStmt = mysqli_prepare($conn, "UPDATE users SET password = ? WHERE id = ? AND role = 'vet' LIMIT 1");
                if ($updateStmt) {
                    mysqli_stmt_bind_param($updateStmt, 'si', $newHash, $resetRow['vet_id']);
                    if (mysqli_stmt_execute($updateStmt)) {
                        $markStmt = mysqli_prepare($conn, "UPDATE vet_password_resets SET used_at = NOW() WHERE id = ? LIMIT 1");
                        if ($markStmt) {
            ...
            $error = 'Unable to verify vet account.';
        }
    }
}
...
<div class="auth-wrapper">
  <div class="auth-card">
    <div class="text-center mb-4">
      ...
      <p class="auth-subtitle">Set a new veterinarian password</p>
    </div>
