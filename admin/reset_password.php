...
if ($error === '' && $token !== '' && $email !== '') {
    $stmt = mysqli_prepare(
        $conn,
        "SELECT id, admin_id, email, token_hash, expires_at
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $showForm && $resetRow && $rate_error === '' && $csrf_valid) {
    ...
    if ($new_password === '' || $confirm_password === '') {
        ...
        $userStmt = mysqli_prepare($conn, "SELECT password FROM users WHERE id = ? AND role = 'admin' LIMIT 1");
        if ($userStmt) {
            mysqli_stmt_bind_param($userStmt, 'i', $resetRow['admin_id']);
            ...
            if (!$userRow) {
                ...
                $updateStmt = mysqli_prepare($conn, "UPDATE users SET password = ? WHERE id = ? AND role = 'admin' LIMIT 1");
                if ($updateStmt) {
                    mysqli_stmt_bind_param($updateStmt, 'si', $newHash, $resetRow['admin_id']);
                    if (mysqli_stmt_execute($updateStmt)) {
                        $markStmt = mysqli_prepare($conn, "UPDATE admin_password_resets SET used_at = NOW() WHERE id = ? LIMIT 1");
                        if ($markStmt) {
...
<head>
  ...
  <title>Reset Password — PetCare HMS</title>
...
<div class="auth-wrapper">
  <div class="auth-card">
    <div class="text-center mb-4">
      ...
      <p class="auth-subtitle">Set a new admin password</p>
